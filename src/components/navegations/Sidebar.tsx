...
import { AddressIcon, BlockActiveIcon, BlockIcon, HomeActiveIcon, HomeIcon, TokenIcon, TxIcon } from '@/common/icons'
...
function Sidebar() {
  const { activeSidebar, setActiveSidebar, setWidthScreen, widthScreen } = useAppDataContext();
  ...
  return (
    <div className={`${activeSidebar ? 'flex' : 'hidden'} fixed md:w-[210px] h-screen inset-0 z-50 md:z-10 md:relative`}>
      ...
      <div onClick={() => closeSidebar()} className='absolute w-screen inset-0 -z-40 bg-black opacity-60 md:hidden'></div>
    </div>
  )
}
