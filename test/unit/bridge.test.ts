import { RSKBridgeFragment, getAllowedBridgeFragments, formatBridgeWriteMethod, formatBridgeReadMethod, isAllowedBridgeMethod } from "@/common/constants/bridge";
...
describe('Bridge object module', () => {
  describe('Fragment Formatters', () => {
    it('should format write methods correctly', () => {
      ...
      expect(formatted).toEqual({ ...mockWriteFragment, constant: false, stateMutability: 'nonpayable' });
    });
  
    it('should format read methods correctly', () => {
      const formatted = formatBridgeReadMethod(mockReadFragment);
      expect(formatted).toEqual({ ...mockReadFragment, stateMutability: 'view' });
    });
  })

  describe('isAllowedBridgeMethod', () => {
    it('should identify allowed bridge methods', () => {
      expect(isAllowedBridgeMethod('getBtcBlockchainBestChainHeight')).toBe(true);
  ...
  describe('getAllowedBridgeFragments', () => {
    ...
    it('should log an error for an invalid bridgeAbi', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Error while formatting bridge fragments: Invalid bridge ABI'));
    ...
    it('should log an error for unnamed fragments', () => {
      ...
      expect(formattedAbi).toHaveLength(0);
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Error while formatting Bridge ABI fragment: Invalid Bridge fragment. Fragment name is missing'));
    ...
    it('should handle a bridgeAbi containing unallowed methods', () => {
      const bridgeAbi = [mockReadFragment, mockWriteFragment, mockNotAllowedFragment];
      const formattedAbi = getAllowedBridgeFragments(bridgeAbi);
      expect(formattedAbi).toHaveLength(2);
      expect(formattedAbi).toContainEqual(expect.objectContaining({ name: 'getBtcBlockchainBestChainHeight', stateMutability: 'view' }));
      expect(formattedAbi).toContainEqual(expect.objectContaining({ name: 'registerBtcTransaction', stateMutability: 'nonpayable' }));
    });
  });
});